...
_Localization notes: BMAD 五角色（John/Sally/Quinn）与 Neil/Fab Academy/Brandy 等专名保留原文；「为我的作品立项」→ Brief Your Project（立项 = write the brief / make it official）；「需求单」→ the requirements sheet / requirements.md（开工许可证 the building permit）；「范围杀手」→ the Scope Killer（唯一句式 "Would it die without this?"，否决权在学员）；「刁难清单」→ the trouble list（第九章预告术语，本课正式启用）；「改数据/改逻辑/改阈值」→ fix the data / fix the logic / fix the threshold；「接受声明」→ the acceptance note；「换题通道」→ the re-topic lane；「保底题」→ the backup topics；「立项墙/立项仪式」→ the Brief Wall / the briefing ceremony；「查引用」→ the citation check；「改需求不改采购」→ change the need, not the purchase；「开眼界三选」→ the three discovery picks。本课铁律「题是学员的，不是你的」→ "The topic is the student's, not yours."（只提问不给答案）。Neil 创意口径（造商店里买不到的东西、创意从体验里长出来、第一周立 Final Project、二十周每周攒模块、毕业金句"人生项目的开始"）与 Brandy 契约对照页（"她写的叫契约，你写的叫需求单——是同一张东西"）完整保留。「不赌中文渲染」规则天然满足——一切屏幕内容为英文/数字/图形。_
