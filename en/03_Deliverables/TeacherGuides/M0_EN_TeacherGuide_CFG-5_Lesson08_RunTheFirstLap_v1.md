...
_Localization notes: BMAD 5-personas (John/Sally/Winston/Amelia/Quinn) and Brandy/Neil kept in original; '跑通第一圈' -> Run the First Lap (sub-title: Build Your Project's MVP); '最小闭环' -> the smallest closed loop; '三幕节奏' -> the three acts (the start / the build / the review & handoff, the gear-change moves); '站会' -> the stand-up; '领料' -> check out the parts (the check-out sheet, the hot parts); '15 分钟无进展必须举手' -> the 15-minute hand-raise rule; '分诊三句话' -> the triage three-liners; '崩盘协议' -> the crash protocol (the shrink plan); '快手三级加餐' -> the fast-lane ladder; '棒交接单' -> the baton handoff sheet (the comeback anchor); '挑剔用户完整版' -> the Picky User, full edition (3 faults + 5-item list); '顶回句式' -> the push-back prompt; '投喂材料' -> the feeding lines (review must be fed current real state); '最小不是简陋，是专业' -> 'Minimal is not crude — it's professional.'; '评审不可取消，只可换执行者' -> 'The review is never canceled — only its executor changes.' Brandy's case and appendix usage instructions fully preserved. Curriculum reordering context smoothly maintained in scripts. 'No gambling on Chinese rendering' rule naturally satisfied._
