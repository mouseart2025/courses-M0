...
_Localization notes: 专名保留（SenseCraft AI、XIAO ESP32S3 Sense、Codecraft、rock/paper/scissors 标签）；「两种 AI」→ "you tell it, it builds" vs "you show it, it learns"；「归因讨论」→ the attribution talk（落点：it's not dumb — you taught it too little）；「三道闸」→ the three gates；「视觉联动」→ vision-to-action；「只拍手不拍脸」→ hands and objects only — no faces；「慢半秒」→ half a second of lag is normal。本课训练段不走对话式 AI（喂数据），联动段用 Codecraft 对话——结构与中文版完全一致。V1→V3 全部内容决策已吸收（零打印、三格单投屏、合规口径、附录加餐独立成章）。_
