...
_Localization notes: Proper names kept (aily-blockly / Grove Beginner Kit / Wio Terminal); '从租户到主人' -> from tenant to owner ('Moving isn't a step back — it's buying the house.'); '确认归属仪式' -> the ownership ritual; '双视图时刻' -> the two-views moment (blocks view <-> code view); '工程师的循环' -> the engineer's loop; '天黑自动亮灯' -> the night-light project; '天黑警报站' -> the dusk alarm station (the lookout / the alarm post); '信号线＋共地' -> the signal wire + shared ground ('no shared ground, no link'); '补装窗' -> the install window; '收口转场' -> the install checkpoint (minute-18 red line). Source revision decisions fully absorbed: install as pre-class homework, 14:18 hard transition, 30-min two-views block cannot be compressed, NLHD loop as the new main dish, zero printing, de-cardified, no unified breaks, ask-code/say-need in same window, Grove single-wire framing. 'No gambling on Chinese rendering' rule naturally satisfied._
